...
import { User, Stock, OrderExecution, Holding, Funds } from '@/lib/types';
...
const dhanMock = {
    ...
    // Mock funds response
    getFunds: (userId: string): Funds => {
        const availablecash = Math.floor(Math.random() * 500000) + 100000;
        const intraday = Math.floor(Math.random() * 1000000) + 200000;
        return {
            availablecash,
            collateral: 0,
            intraday,
            total: availablecash + intraday
        };
    }
};
...
export async function POST(request: NextRequest, { params }: { params: { path?: string[] } }) {
    try {
        ...
        if (path === 'auth/register') {
            ...
            // Store password separately or in user object (in real app, hash it)
            await database.collection('users').insertOne({ ...user, password, createdAt: new Date() });
        ...
        if (path === 'auth/login') {
            ...
            const userDoc = await database.collection('users').findOne({ email, password });
            if (!userDoc) {
        ...
        if (path === 'admin/execute-order') {
            ...
            const executionResults: OrderExecution[] = [];
            ...
            for (const subscriber of activeUsers) {
                try {
                    ...
                    if (transactionType === 'BUY') {
                        ...
                        const capitalToUse = (subscriber.maxCapital || 0) * 0.25;
                    ...
                    if (canExecute) {
                        ...
                        if (dhanConnection && dhanConnection.accessToken && dhanConnection.clientId) {
                            try {
        ...
        if (path === 'user/connect-broker') {
            ...
            if (brokerName === 'Dhan' && accessToken && clientId) {
                try {
                    ...
                    await dhanBroker.getFunds(); // Verify connection
            ...
            await database.collection('users').updateOne(
                { id: user.id },
                { $push: { brokerConnections: brokerConnection } as any }
            );
